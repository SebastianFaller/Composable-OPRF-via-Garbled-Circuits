...
inline void init(Bit * bits, const bool* b, int length, int party) {
	...
	if (party == PUBLIC) {
		Label one = CircuitExecution::circ_exec->public_label(true);
		Label zero = CircuitExecution::circ_exec->public_label(false);
		for(int i = 0; i < length; ++i)
			bits[i].bit0 = b[i] ? one : zero;
	}
	else {
        for(int i = 0; i < length; i++)
            ProtocolExecution::prot_exec->feed(&(bits[i].bit0), &(bits[i].bit1), party, b + i, 1); 
	}
}
