...
def stringToBitList(s):
    ...
    for c in s:
        for i in range(8):
            l.append((ord(c) >> (8 - i - 1)) % 2)
    return l

def sendAndRecData(data, HOST, PORT):
...
def runUser(host, port):
    ...
    for i in range(nr_iterations):
        ...
        start = time.time()
        bits = stringToBitList(input_string)

        s = voprf.sample_small(scalar = True)
        e1 = voprf.sample_small(scalar = False)
        cx = voprf.bp14(bits) + voprf.a[2] * s + e1

#        # Just for testing!
#        cx = ZZ["x"]

        print("sending...")
        print("size " + str(len(dumps(cx))))
        response = sendAndRecData(dumps(cx), host, port)
        dx = loads(response)

        y = voprf.nice_lift((dx - voprf.c * s)[0])
        output =  y // (voprf.q / voprf.p)

        measured = time.time() - start
        time_stamps.append(measured*1000.0) # milliseconds
